...
import type { Scrollable } from '../shared/scroll';
...
/** Radians added to every moon's own angle per loop() call, independent of scroll — without this the
 *  whole orbit sat frozen until a viewer actually scrolled (`rotation` below only ever moved via
 *  `scrollPosition`'s setter). This is added on top, not blended into `rotation` itself, so it can't
 *  disturb Scroll's own read/write contract on scrollPosition (see loop()'s own comment). Small and
 *  per-tick rather than delta-scaled — matches this page's own rAF loop, which (like scroll.ts's own
 *  loop()) doesn't track real elapsed time either; at a typical 60fps tick rate this is one full
 *  revolution roughly every 3 minutes, a slow ambient drift rather than a deliberate spin. */
const IDLE_ROTATION_STEP = 0.0006;
...
export class SkillMoons implements Scrollable {
	...
	private rotation = 0;
	/** See IDLE_ROTATION_STEP's own comment — a separate accumulator from `rotation` (scroll-driven),
	 *  added alongside it in recomputePositions() rather than into it. */
	...
	/** Call once per frame (alongside scroll.loop() — see skills/+page.svelte) for the idle ambient
	 *  drift — see IDLE_ROTATION_STEP's own comment for why this exists and why it's a separate
	 *  accumulator from the scroll-driven `rotation`. */
	loop(): void {
		this.idleRotation += IDLE_ROTATION_STEP;
		this.recomputePositions();
	}

	private recomputePositions(): void {
		const totalRotation = this.rotation + this.idleRotation;
		for (let i = 0; i < this.baseAngles.length; i++) {
			const angle = this.baseAngles[i] + totalRotation;
