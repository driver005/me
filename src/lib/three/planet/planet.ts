 ...
 *  where this class's shaders came from). 'about' was named 'info' there and in this port until it was
 *  renamed to match our own actual /about route (the source's own site had no equivalent route name to
 *  clash with; ours does, and 'info' read as unrelated to a small "info" LINK label that also exists
 *  elsewhere — see common.info_link, a separate, un-renamed thing). 'error' exists for parity with the
 *  source but nothing in this port currently routes to it. 'skills' was added later, for Compositor's
 *  own PAGE_LOOK table (glow/fog tuning specific to /skills) — that route renders through
 *  RaymarchPlanet (earthPlanet), never through THIS mesh-based Planet class, so PLANET_PAGES.skills
 *  below (required to keep this type's own Record exhaustive) is dead data, same as 'error's entry. */
export type PlanetPageId = 'home' | 'work' | 'about' | 'skills' | 'error';
...
const PLANET_PAGES: Record<PlanetPageId, PlanetPageConfig> = {
	...
	// Dead data — see PlanetPageId's own comment. Copied from 'about' rather than left as zeros so it's
	// at least a sane value if something ever does route /skills through this class.
	skills: {
		position: { x: -38, y: -40, z: -26.6 },
		scale: 150,
		uniforms: { uGlowBiasX: 0.6, uRimPow: 4.2, uGlowPow: 0.5, uGlowStr: 0.6, uRimStr: 1, uTerrainScale: 3.9 }
