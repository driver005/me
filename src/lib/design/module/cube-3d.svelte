...
>
	<Motion
		...
		transition={{
			rotateY: {
				duration: shouldAnimate ? 22 : 0.01,
				ease: 'linear',
				repeat: shouldAnimate ? Infinity : 0
			},
			rotateX: {
				duration: shouldAnimate ? 32 : 0.01,
				ease: 'linear',
				repeat: shouldAnimate ? Infinity : 0
			}
		}}
		let:motion
	>
		<div use:motion class="relative h-full w-full" style="transform-style: preserve-3d;">
			<Motion style={{ rotateX: smx, rotateY: smy }} let:motion>
				...
				>
					{#each FACES as f}
						<div
							class="absolute inset-0 overflow-hidden border border-black bg-[#E5E4E0]"
							style="transform: {f.tx} translateZ({half}px); backface-visibility: hidden;"
						>
							<img src={f.img} alt={f.label} class="h-full w-full object-cover" draggable={false} />
							<span
								class="absolute bottom-1 left-1 bg-[#F3F2EE] px-1.5 py-0.5 font-mono text-[9px] tracking-[0.25em] uppercase"
							>
								{f.label}
							</span>
						</div>
