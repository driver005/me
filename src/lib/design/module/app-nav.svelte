<script lang="ts">
	...
	import { socialItems, languages } from '$lib/data';
	...
	function linkClass(href: string) {
		...
		const base =
			'font-mono text-xs uppercase tracking-[0.2em] group relative overflow-hidden inline-flex flex-col';
	...
	function socialClass() {
		return darkMode
			? 'text-[#F3F2EE]/70 hover:text-[#FF3B00] transition-colors'
			: 'text-[#555] hover:text-[#FF3B00] transition-colors';
	}
			...
			(entries) => {
				for (const entry of entries) {
					if (entry.intersectionRatio > 0 && entry.intersectionRatio < 1) {
						const isDark =
							darkSelectors.some((s) => entry.target.matches(s)) ||
...
{#if show}
	<header
		class="fixed top-0 right-0 left-0 z-50 border-b backdrop-blur-sm transition-colors duration-300 {darkMode
			? 'border-[#F3F2EE]/20 bg-[#0A0A0A]/90'
			: 'border-black bg-[#F3F2EE]/90'}"
	...
	>
		<div class="flex h-14 items-center justify-between px-4 sm:px-8">
			...
			<nav class="hidden items-center gap-6 md:flex">
				{#each routeLinks as l}
					<a href={l.href} class={linkClass(l.href)} style="height:1.1em;line-height:1.1em">
						<span
			...
			<div class="hidden items-center gap-4 md:flex">
				{#if currentPath !== '/'}
					<div
						class="flex items-center gap-3 border-r pr-4 {darkMode
							? 'border-[#F3F2EE]/20'
							: 'border-black'}"
					>
				...
				<span class="hidden items-center gap-1.5 lg:flex">
					<span class="h-1.5 w-1.5 animate-pulse rounded-full bg-[#22c55e]"></span>
					<span class="font-mono text-[10px] tracking-[0.2em] text-[#22c55e] uppercase"
						>{m['nav.open']()}</span
					>
				</span>
				<span
					class="font-mono text-xs tracking-[0.2em] uppercase {darkMode
						? 'text-[#F3F2EE]/70'
						: 'text-[#555]'}">{time}</span
				>
				<div class="flex items-center gap-1 font-mono text-xs tracking-[0.2em] uppercase">
					{#each languages as lang, i}
						{#if i > 0}<span class={darkMode ? 'text-[#F3F2EE]/20' : 'text-[#0A0A0A]/20'}>|</span
							>{/if}
						<button
							onclick={() => setLocale(lang.id as 'en' | 'de')}
							class="transition-colors duration-300 {getLocale() === lang.id
								? 'text-[#FF3B00]'
								: darkMode
									? 'text-[#F3F2EE]/50 hover:text-[#F3F2EE]'
				...
				<MagneticButton strength={0.4}>
					<a
						href="mailto:{m.email()}"
						class="inline-block px-3 py-1.5 font-mono text-xs tracking-[0.2em] uppercase transition-colors {darkMode
							? 'bg-[#F3F2EE] text-[#0A0A0A] hover:bg-[#FF3B00]'
							: 'bg-[#0A0A0A] text-[#F3F2EE] hover:bg-[#FF3B00]'}"
					>
						{m['nav.cta']()}
					</a>
				</MagneticButton>
			...
			<button
				class="border px-3 py-1.5 font-mono text-xs tracking-[0.2em] uppercase transition-transform active:scale-95 md:hidden {darkMode
					? 'border-[#F3F2EE]/40 text-[#F3F2EE]'
		...
		{#if open}
			<nav
				...
				class="border-t bg-[#F3F2EE] md:hidden {darkMode
					? 'border-[#F3F2EE]/20 bg-[#0A0A0A]'
					: 'border-black'}"
			>
				<div class="flex flex-col gap-3 p-4">
					{#each routeLinks as l}
						<a href={l.href} onclick={() => (open = false)} class={mobileLinkClass(l.href)}>
							→ {l.label}
						</a>
					{/each}
					{#if currentPath !== '/'}
						<div
							class="flex gap-4 border-t pt-3 {darkMode
								? 'border-[#F3F2EE]/20'
								: 'border-black/20'}"
						>
							{#each socialItems as s}
					...
					{/if}
					<div
						class="flex items-center gap-2 border-t pt-3 font-mono text-sm tracking-[0.2em] uppercase {darkMode
							? 'border-[#F3F2EE]/20'
							: 'border-black/20'}"
					>
						{#each languages as lang, i}
