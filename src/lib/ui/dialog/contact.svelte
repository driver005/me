...
<Dialog.Root bind:open>
	<Dialog.Trigger>
		<Button
			variant="outline"
			onclick={() => (open = true)}
			class="h-12 rounded-xl border-4 border-black bg-orange-400 brutal-shadow transition-all hover:bg-orange-400 active:translate-x-0.5 active:translate-y-0.5 active:shadow-none dark:bg-orange-400 dark:hover:bg-orange-400"
		>
		...
		<div class="grid grid-cols-2 gap-4 py-4">
			{#each dock.contact as link}
				<a
					...
					onclick={() => (open = false)}
					class="hover:shadow-nonw flex items-center gap-3 rounded-xl border-4 border-black p-3 font-black brutal-shadow transition-all duration-300 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none {link.color}"
				>
