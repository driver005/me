<script lang="ts">
	...
	let cursorMode = $derived(
		cursorLabel === 'view' ? 'view' :
		cursorLabel === 'drag' ? 'drag' :
		cursorLabel === 'link' ? 'link' :
		hover ? 'hover' : 'default'
	);
...
<Motion style={{ translateX: springX, translateY: springY }} let:motion>
	<div use:motion data-testid="custom-cursor" aria-hidden="true" class="pointer-events-none fixed top-0 left-0 z-[100] hidden md:block">
		<Motion
			...
			let:motion
		>
			<div use:motion class="-translate-x-1/2 -translate-y-1/2 h-3 w-3 rounded-full"></div>
		</Motion>
		{#if cursorLabel}
			<span
				class="absolute -translate-x-1/2 -translate-y-1/2 font-mono text-[9px] uppercase tracking-wider text-white whitespace-nowrap pointer-events-none"
...
<Motion style={{ translateX: springGhostX, translateY: springGhostY }} let:motion>
	...
	>
		<Motion
			...
			let:motion
		>
			<div use:motion class="-translate-x-1/2 -translate-y-1/2 h-8 w-8 rounded-full border"></div>
		</Motion>
	</div>
</Motion>
