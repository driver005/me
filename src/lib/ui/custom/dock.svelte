...
<Tooltip.Provider delayDuration={200}>
	...
	>
		{#snippet children(mouseX: number, magnification: number, distance: number)}
			{#each dock.navbar as item}
				...
				>
					<Tooltip.Root>
						<Tooltip.Trigger
							class={`flex cursor-pointer items-center justify-center rounded-xl border-2 border-black ${item.color} p-3 brutal-shadow transition-transform transition-shadow duration-500 ease-[var(--ease-out-expo)] hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none dark:text-black`}
						>
