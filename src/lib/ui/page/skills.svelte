...
<div class="grow overflow-x-hidden overflow-y-auto bg-background p-6 lg:p-10">
	<div class="mx-auto flex w-full flex-col items-center">
		...
		>
			...
			{#each skill_tree.categories as category}
				<div class="flex w-full shrink flex-col items-center lg:flex-1">
					...
					<div class="grid w-full grid-cols-1 gap-3 px-2 sm:grid-cols-2 lg:grid-cols-1 lg:px-0">
						{#each category.skills as skill}
							<div
								class={`group flex flex-col rounded-xl border-4 p-4 transition-all ${skill.colorClass}  shadow-[4px_4px_0px_0px_currentColor] transition-all duration-700 hover:translate-x-0.5 hover:translate-y-0.5 hover:shadow-none `}
							>
