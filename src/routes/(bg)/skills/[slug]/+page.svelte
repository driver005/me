...
<svelte:head>
	<title>{skill ? `${skill.name} — Adrian Fernández` : 'Skill not found'}</title>
</svelte:head>

{#if skill}
	<div class="pointer-events-none fixed inset-x-0 bottom-0 z-20 flex flex-col gap-3 p-8 text-white sm:max-w-xl">
		<a href="/skills" class="pointer-events-auto w-fit text-sm text-white/60 underline hover:text-white">← Back</a>
		...
		<div class="mt-2 grid gap-3 sm:grid-cols-2">
			<div class="rounded-lg border border-emerald-400/30 bg-emerald-400/10 p-3">
				<p class="text-xs font-bold tracking-widest text-emerald-300 uppercase">Pro</p>
				<p class="mt-1 text-sm text-white/85">{pro}</p>
			</div>
			<div class="rounded-lg border border-red-400/30 bg-red-400/10 p-3">
				<p class="text-xs font-bold tracking-widest text-red-300 uppercase">Contra</p>
...
{:else}
	<div class="pointer-events-none fixed inset-x-0 bottom-0 z-20 p-8 text-white">
		<a href="/skills" class="pointer-events-auto underline">← Back — skill not found</a>
	</div>
{/if}
